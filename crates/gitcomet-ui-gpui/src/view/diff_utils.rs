...
pub(super) fn compute_diff_word_highlights(
    ...
    while ix < diff.len() {
        ...
        if matches!(kind, gitcomet_core::domain::DiffLineKind::Hunk) {
            ix += 1;
            continue;
        }

        if !matches!(kind, gitcomet_core::domain::DiffLineKind::Remove) {
            ix += 1;
            continue;
        }
        ...
        while ix < diff.len() && matches!(diff[ix].kind, gitcomet_core::domain::DiffLineKind::Add) {
            let text = diff_content_text(&diff[ix]);
            added.push((ix, text));
            ix += 1;
        }
